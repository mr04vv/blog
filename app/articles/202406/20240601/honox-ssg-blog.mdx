...
rendererはデフォルトでは `hono/jsx`のものが使われますが、[README](https://github.com/honojs/honox/tree/main?tab=readme-ov-file#byor---bring-your-own-renderer)に書かれているように、React等のrendereを使うこともできるみたいです。が、今回は試してません。
...
今回はSSGで事前にHtmlを生成するようにしたのですが、記事のタイトルで変化するOGPも自動生成することにしました。
...
結果的にmdxから記事のタイトルを抽出したOGPを生成してくれるようになりました。
