...
date: 2024-06-23T00:25:11.375Z
description: 
iconUrl: https://raw.githubusercontent.com/microsoft/fluentui-emoji/main/assets/Cat/Flat/cat_flat.svg
---

少し前にCloudflare PagesとHonoで愛猫「らて」のLGTM画像サイトを作成したのですが、画像のメンテナンスコストが高すぎたのでCMSへ移行することにしました。
...
構成は以下のような感じで、Contentfulへの画像アップロードをトリガーにして、Vercelのビルドを走らせるようにしています。
...
Contentfulの全体像は[こちら](https://dev.classmethod.jp/articles/contentful/)の記事を参照してください。
<ExternalOgp url="https://dev.classmethod.jp/articles/contentful/"/>

Contentfulのアカウントを作成するとBlankという名前のSpaceが作られているのでそれを使います。
...
Github Actionsでのビルドも同様にContentful側の変更をトリガーにしてWebhook経由で実行できるみたいです。
