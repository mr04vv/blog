---
title: リアルタイム共同編集を実現できるLoroを試してみる
...
この記事は[Atrae Advent Calendar 2024](https://adventar.org/calendars/10486)の6日目の記事です。
...
同じような仕組みを使ったライブラリには、他にも [Yjs](https://github.com/yjs/yjs) や [Automerge](https://github.com/automerge/automerge) といったものがあります。

タイムトラベル(履歴？スナップショット？)機能が簡単に実装できるのも特徴らしいです。
...
今回はホワイトボードライブラリの [tldraw](https://github.com/tldraw/tldraw) を使って、共同編集可能なホワイトボードを作ってみます。
[サンプルコード](https://github.com/mr04vv/tldraw-loro/tree/main)を用意しましたのでぜひ参考にしてみてください。
<ExternalOgp url="https://github.com/tldraw/tldraw"/>

まずはLoroの差分を垂れ流すだけのWebsocketサーバーを作ります。
...
続いてクライアントです。クライアントの全貌はGitHubに用意したのでそちらをご覧ください。
...
ここではWebsocketサーバーから受け取った差分をクライアント側のdocにimportしています。7行目で受け取ったタイミングのバージョンを保持しておきます。そうすることで更新時に差分のみサーバーへ送信できるようになります。
...
処理の流れとしては以下のような感じです。
1. [tldrawの状態の変化を検知する](https://github.com/mr04vv/tldraw-loro/blob/664dcb660250ba245c6ab1de617782a1890363c4/apps/client/src/App.tsx#L135-L151)
...
Loroのではsubscribeメソッドが用意されていて、イベントヘンドラをセットできます。
イベントハンドラ内では、ローカルでの更新とリモートの更新（importメソッドを使用した更新）を区別できるので、以下のようにそれぞれ処理を分けることができます。
...
Cloudflareの[Durable Objects](https://developers.cloudflare.com/durable-objects/)と組み合わせて使えたらかなり面白そう。時間あるときに試してみようかな…？
