...
この記事は[Atrae Advent Calendar 2024](https://adventar.org/calendars/10486)の18日目の記事です。前回は[@aki90935433](https://x.com/aki90935433)による「[hogehoge]()」でした。
...
今回は [`motion`](https://motion.dev/docs/quick-start)を使ったデモを用意したのでアニメーションのON/OFFを切り替えながら体感してみてください。(スマホだと多分動かないです)

デモでは140msでThrottleしています。

<CursorDemo/>

こんな感じでアニメーションを付けると、負荷を下げつつ滑らかなカーソル同期が実現できます。
...
普段のフロントエンドより、一層パフォーマンスに気をつけなければならないので今後も面白い技術があったら書いていこうと思います。
...
liveblocksの記事がとても参考になるのでぜひご覧ください。デモもあるのでわかりやすいです。
